...
**Next implementation:** follow `docs/weather/R_ADAPTATION_PLAN.md` milestones A–B and the full-review gates: isolate reviewed R science, adapt existing point data, and produce the Waldo open-ground strategy/calendar/stage-risk packet, then Citra/Papanduva. Fix calendar anchors, missingness, stable row schemas and keyed identities; distinguish both classifiers and require an explicit anchor for a zero-chill scenario. Do not port the flawed composite suitability/analogue outputs unchanged. Do not make cultivar data, additional soil or tunnel models prerequisites. Expanded hourly evaluation needs a finite registry before acquisition; no global hourly download.
...
Latest action: completed the full sequential private R read, documented all sections/functions and extended the audit from eight to 22 passing checks. The prior 55-test server suite was not rerun this turn. Locally R 4.6.1 exists; no dependencies installed or server R check this turn. No production fixes, acquisition, scientific model or deployment. Publication result is reported in the task response, not assumed here.
