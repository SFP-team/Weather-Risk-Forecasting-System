...
Supervisor-review amendment, 2026-09-11: next add **production-system hypothesis → seasonal calendar → stage-specific risks** for open field + ground, using the R adaptation plan in `weather/R_ADAPTATION_PLAN.md`. Keep annual climate and mapped soil as supporting evidence. Do not prioritize quantitative tunnel/pot comparison or additional decorative UI before evaluating the Waldo packet. Production strategy (evergreen/semi/deciduous) is a different field from the existing growing-setup selector. Plan the R adapter alongside, not as a replacement for, existing Python archive/QC and daily summaries.
...
- **Science:** retain tested Python extraction, QC and daily summaries. For the next production outputs, isolate reviewed R functions behind a versioned point-data contract as specified in `weather/R_ADAPTATION_PLAN.md`. Existing Python 50/100-hour scenarios provide parity tests. Do not source Paul's whole workflow, duplicate acquisition, or maintain two independently evolving production-calendar implementations.
