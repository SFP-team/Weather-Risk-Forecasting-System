...
Updated 2026-09-23 with geographic search and the supplied gallery design. This is a historical research workspace, not a cultivar recommendation or weather forecast. The vanilla-JavaScript frontend uses the lab logo, system fonts, white/gray surfaces, black type, restrained blue pill controls and 28px shadowless cards/map. The current design contract is in `UI_IMPLEMENTATION_PLAN.md`.
